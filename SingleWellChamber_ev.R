...
## THIS HAS BEEN MODIFIED FOR AVERAGE EVENT DURATION, SEE FUNCTION BINFEEDINGDATA.WELL.EVENTS()
source("DFM.R")
...
BinFeedingData.Events<-function(dfm,binsize.min,range=c(0,0)){
  ...
  for(i in 2:12) {
    ...
    result<-data.frame(result,tmp$MeanEventDur)
  }
  names(result)<-c("Interval","Min",paste("W",1:12,sep=""))
  result  
}

BinFeedingData.Well.Intervals<-function(dfm,well,binsize.min,range=c(0,0)){
  tmp<-FeedingData.Licks(dfm,range)
  ints<-Feeding.Intervals.Well(dfm,well)
  cname=paste("W",well,sep="")
 
  new<-matrix(0,length(tmp$Sample),1)
  
  for(x in 1:length(ints$Sample)){
    new[ints$Sample[x]+1]<-ints$IntervalSec[x]
  }
  ...
  tempMins<-as.numeric(tmp$Minutes)

  z<-cut(tempMins,y,include.lowest=TRUE)
  ...
  results<-data.frame(r.min,r.A[,2])
  names(results)<-c("Interval","Min","MeanIntervalDur")
  results
}

BinFeedingData.Intervals<-function(dfm,binsize.min,range=c(0,0)){
  result<-BinFeedingData.Well.Intervals(dfm,1,binsize.min,range)
  
  for(i in 2:12) {
    cname=paste("W",i,sep="")
    tmp<-BinFeedingData.Well.Intervals(dfm,i,binsize.min,range)
    result<-data.frame(result,tmp$MeanIntervalDur)
  }
  names(result)<-c("Interval","Min",paste("W",1:12,sep=""))
  result  
}
